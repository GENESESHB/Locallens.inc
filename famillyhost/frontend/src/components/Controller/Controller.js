...
const Controller = () => {
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      const updatedResponse = await fetch(`http://localhost:5000/api/user/${userId}`, {
        headers: {
      ...
      updateUser(updatedUserData); // Update the user in the context
        ...
        {activeCard === 1 && (
          <div className="card1">
            <div className="addservice">
              <button onClick={handleMediaClick} className="mediainfo"><FaCamera /> أخبار عامة<span> زيادة أخبار عامة</span></button>
              <button onClick={handleSportClick} className="sportinfo"><FaFutbol /> أخبار الرياضة<span> زيادة أخبار اررياضة</span></button>
              <button onClick={handleMeteoClick} className="meteoinfo"><FaCloud /> أخبار الطقس<span> زيادة أخبار الطقس</span></button>
            </div>
            <div className="items">
