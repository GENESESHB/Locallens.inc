...
function MediaServices() {
  ...
  const fetchMediaServices = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/user/${user._id}/meteoservices`, {
        method: 'GET',
        credentials: 'include',
      });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/user/meteoservices', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
  ...
  const handleDeleteService = async (serviceId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/user/${user._id}/meteoservices/${serviceId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      ...
      if (response.ok) {
        ...
        console.error('Failed to delete media service');
      }
    } catch (error) {
