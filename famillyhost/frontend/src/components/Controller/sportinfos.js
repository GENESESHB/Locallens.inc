...
function MediaServices() {
  ...
  const fetchMediaServices = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/user/${user._id}/sportservices`, {
        method: 'GET',
        credentials: 'include',
      });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/user/sportservices', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
  ...
  const handleDeleteService = async (serviceId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/user/${user._id}/sportservices/${serviceId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
